-module(shareware_definition_supervisor).
...
-type t() :: #{
    id => shareware_definition_child_spec:child_id(),
    flags := supervisor:sup_flags(),
    children := [
        supervisor:child_spec()
        | shareware_definition:ref()
...
expand(ID, #{flags := Flags, children := Children} = Spec, Container, Visited) ->
    ...
    shareware_definition_child_spec:expand(ID, ChildSpec, Container, Visited).
...
expand_children(Children, Container, Visited) ->
    F = fun
        ...
        (?DEFINITION(Mod, _) = Definition) when
            Mod =:= ?MODULE orelse Mod =:= shareware_definition_child_spec
        ->
            shareware_definition:expand(
                undefined, Definition, Container, Visited
            );

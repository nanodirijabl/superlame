-module(stdm_definition_supervisor).
...
-type t() :: #{
    id => stdm_definition_child_spec:child_id(),
...
expand(ID, #{flags := Flags, children := Children} = Spec, Container, Visited) ->
    ...
    stdm_definition_child_spec:expand(ID, ChildSpec, Container, Visited).
...
expand_children(Children, Container, Visited) ->
    F = fun
        ...
        (?DEFINITION(Mod, _) = Definition) when
            Mod =:= ?MODULE orelse Mod =:= stdm_definition_child_spec
        ->
            stdm_definition:expand(undefined, Definition, Container, Visited);
